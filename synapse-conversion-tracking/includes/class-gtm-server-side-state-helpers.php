...
class GTM_Server_Side_State_Helpers {
	...
	const CART_TOKEN_COOKIE = '_synapse_cart_state_token';
	...
	private function get_cart_items( $cart ) {
		...
		foreach ( $cart as $product_loop ) {
			...
			$product_data = array(
				...
				'item_name'        => GTM_Server_Side_WC_Helpers::instance()->sanitize_payload_text( $product->get_name() ),
	...
	public static function get_cart_token() {
		// The session handler is only loaded on frontend-shaped requests, so it
		// can legitimately be null even when WooCommerce itself is active.
